...
app.get("/health",(_req,res)=>res.json({ok:true,service:"gbi-research-worker",version:"0.1.1",time:new Date().toISOString()}));
...
app.post("/jobs",auth,(req,res)=>{
  ...
  const job:DiscoveryJob={
    id,type:parsed.data.type,seed:parsed.data.seed,country:parsed.data.country,
    status:"queued",created_at:new Date().toISOString(),results:[]
  };
  jobs.set(id,job);
  ...
  setImmediate(async()=>{
    ...
    try{
      ...
      job.status=out.status; job.message=out.message; job.results=out.results;
    }catch(e){
      job.status="failed";
      job.message=e instanceof Error?e.message:"Unhandled worker error";
    }finally{
...
app.listen(port,"0.0.0.0",()=>console.log(`GBI Research Worker v0.1.1 listening on :${port}`));
