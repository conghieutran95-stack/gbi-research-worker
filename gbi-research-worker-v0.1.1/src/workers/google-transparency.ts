...
async function trySearch(page:Page,seed:string):Promise<boolean>{
  ...
  for(const selector of selectors){
    try{
      ...
      if(await loc.isVisible({timeout:1200})){ await loc.fill(seed); await loc.press("Enter"); return true; }
    }catch{}
  }
  return false;
}
...
export async function runGoogleAdsTransparency(seed:string,country?:string):Promise<{
  ...
  try{
    ...
    if(!searched) return {status:"manual_required",message:"Search control not found; Transparency Center UI may have changed.",results:[]};
    ...
    for(const url of urls){
      ...
      seen.add(domain);
      results.push({
        provider:"google_ads_transparency",domain,landing_url:url,country,
        source_url:page.url(),source_ref:seed,observed_at:new Date().toISOString(),
    ...
    if(!results.length) return {
      ...
      message:"Search ran, but no publicly rendered external landing domains were extractable.",
      results:[]
    };
